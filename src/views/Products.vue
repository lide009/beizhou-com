<template>
  <div class="min-h-screen bg-gray-50 pt-20">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-16">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white rounded-xl shadow-md p-6 md:p-8">
              ...
              <div class="space-y-3 text-gray-600">
                <div class="flex items-start gap-3">
                  <span class="w-20 text-sm font-semibold text-gray-500 flex-shrink-0">产品类型</span>
                  <span>环氧基负性光刻胶</span>
                </div>
                <div class="flex items-start gap-3">
                ...
                <div class="flex items-start gap-3">
                  <span class="w-20 text-sm font-semibold text-gray-500 flex-shrink-0">兼容性</span>
                  <span>与进口SU-8工艺高度兼容，可快速导入现有工艺线。支持多层套刻工艺，无需大规模工艺重构</span>
                </div>
              </div>
            </div>

            <div class="bg-gradient-to-r from-blue-50 to-blue-100 rounded-xl shadow-sm p-6 md:p-8 border border-blue-200">
              ...
              <p class="text-gray-700">
                产品性能经<strong>北京大学微加工与微流控平台教授课题组</strong>测试验证并认可。使用说明书由相关专家团队指导编撰，内容全面、细节清晰、实操性强。
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div v-for="item in advantages" :key="item.icon" class="bg-white rounded-xl shadow-md p-5 hover:shadow-lg transition-shadow border-t-4 border-accent">
      ...
      <!-- ===== Section 3: 产品型号参数表 ===== -->
      <section>
        <h2 class="text-2xl font-bold text-gray-800 mb-6 text-center">产品型号与参数</h2>
            ...
            <table class="w-full text-sm md:text-base">
              <thead>
                <tr class="bg-gradient-to-r from-primary to-secondary text-white">
                  ...
                  <th class="px-6 py-4 text-left font-semibold">匀胶厚度范围</th>
                  <th class="px-6 py-4 text-left font-semibold">典型应用</th>
                </tr>
              </thead>
              <tbody>
                <tr v-for="(p, i) in productModels" :key="p.model" class="border-b border-gray-100 hover:bg-blue-50/50 transition-colors" :class="i % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'">
                  <td class="px-6 py-4 font-semibold text-primary">{{ p.model }}</td>
                  <td class="px-6 py-4 text-gray-700">{{ p.thickness }}</td>
                  <td class="px-6 py-4 text-gray-700">{{ p.application }}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
              ...
              <table class="w-full text-sm md:text-base">
                <thead>
                  <tr class="bg-gray-100">
                    <th class="px-6 py-4 text-left font-semibold text-gray-700">规格</th>
                    <th class="px-6 py-4 text-left font-semibold text-primary">北州SU-8光刻胶</th>
                    <th class="px-6 py-4 text-left font-semibold text-gray-500">进口SU-8光刻胶</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(row, i) in priceComparison" :key="row.spec" class="border-b border-gray-100" :class="i % 2 === 0 ? 'bg-white' : 'bg-blue-50/30'">
                    <td class="px-6 py-4 font-medium text-gray-700">{{ row.spec }}</td>
                    <td class="px-6 py-4 text-primary font-semibold">{{ row.beizhou }}</td>
                    <td class="px-6 py-4 text-gray-500">{{ row.importPrice }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div class="lg:col-span-2 space-y-4">
            <div class="bg-gradient-to-br from-primary to-secondary text-white rounded-xl shadow-lg p-6 text-center">
              <div class="text-4xl font-extrabold mb-1">1/4</div>
              <p class="text-blue-100 text-sm">价格仅为进口SU-8的 1/4</p>
              <p class="text-blue-200 text-xs mt-1">大幅降低研发试错成本，提高量产性价比</p>
            </div>
            <div class="bg-white rounded-xl shadow-md p-5 border border-green-200">
              ...
              <div class="flex items-baseline gap-4">
                <div class="text-center">
                  <span class="text-2xl font-extrabold text-green-600">2 周</span>
                  <p class="text-xs text-gray-500">北州SU-8</p>
                </div>
                <span class="text-gray-400">vs</span>
                <div class="text-center">
                  <span class="text-2xl font-bold text-gray-400">2 个月+</span>
                  <p class="text-xs text-gray-500">进口SU-8</p>
                </div>
              </div>
            </div>
            <div class="bg-white rounded-xl shadow-md p-5 border border-blue-200">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-gray-600 text-sm">国产自主供应，不受国际形势和进口限制影响</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          ...
          <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-blue-500">
            <div class="flex items-center gap-2 mb-3">
              ...
              <h3 class="font-bold text-gray-800">个人防护</h3>
            </div>
            <ul class="space-y-2 text-sm text-gray-600">
              <li class="flex items-start gap-2">
                <span class="w-2 h-2 bg-blue-400 rounded-full mt-1.5 flex-shrink-0"></span>
                <span><strong>手部</strong>：丁基橡胶手套</span>
              </li>
              <li class="flex items-start gap-2">
                <span class="w-2 h-2 bg-blue-400 rounded-full mt-1.5 flex-shrink-0"></span>
                <span><strong>眼部</strong>：化学护目镜或面罩</span>
              </li>
              <li class="flex items-start gap-2">
                <span class="w-2 h-2 bg-blue-400 rounded-full mt-1.5 flex-shrink-0"></span>
                <span><strong>身体</strong>：防静电工作服</span>
              </li>
              <li class="flex items-start gap-2">
                <span class="w-2 h-2 bg-blue-400 rounded-full mt-1.5 flex-shrink-0"></span>
                <span><strong>呼吸</strong>：必要时佩戴防有机蒸汽滤毒盒</span>
              </li>
            </ul>
          </div>
          ...
          <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-green-500">
            <div class="flex items-center gap-2 mb-3">
              ...
              <h3 class="font-bold text-gray-800">储存条件</h3>
            </div>
            <ul class="space-y-2 text-sm text-gray-600">
              <li class="flex items-start gap-2">
          ...
          <div class="bg-white rounded-xl shadow-md p-6 border-l-4 border-yellow-500">
            <div class="flex items-center gap-2 mb-3">
              ...
              <h3 class="font-bold text-gray-800">急救措施</h3>
            </div>
            <ul class="space-y-2 text-sm text-gray-600">
              <li class="flex items-start gap-2">
                <span class="w-2 h-2 bg-yellow-400 rounded-full mt-1.5 flex-shrink-0"></span>
                <span><strong>皮肤</strong>：肥皂水冲洗，脱去污染衣物</span>
              </li>
              <li class="flex items-start gap-2">
                <span class="w-2 h-2 bg-yellow-400 rounded-full mt-1.5 flex-shrink-0"></span>
                <span><strong>眼睛</strong>：清水冲洗至少15分钟，立即就医</span>
              </li>
              <li class="flex items-start gap-2">
                <span class="w-2 h-2 bg-yellow-400 rounded-full mt-1.5 flex-shrink-0"></span>
                <span><strong>吸入</strong>：移至空气新鲜处，保持呼吸通畅</span>
              </li>
              <li class="flex items-start gap-2">
                <span class="w-2 h-2 bg-yellow-400 rounded-full mt-1.5 flex-shrink-0"></span>
                <span><strong>食入</strong>：切勿催吐，立即漱口并就医</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      <section>
        <div class="bg-gradient-to-r from-primary to-secondary rounded-2xl shadow-xl overflow-hidden">
          <div class="px-6 py-12 md:px-12 md:py-16 text-center text-white">
            ...
            <h2 class="text-2xl md:text-3xl font-bold mb-4">2026年6月30日前可预定SU-8 BZ3015试用装</h2>
            <p class="text-blue-100 text-lg mb-2">价值 <span class="text-white font-bold text-xl">500 元</span> 的试用装，数量有限</p>
            <p class="text-blue-200 text-sm mb-8">联系技术支持团队获取样品，助力国产替代先行一步</p>
            <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
              <button @click="goContact" class="w-full sm:w-auto px-8 py-3 bg-white text-primary font-bold rounded-lg hover:bg-blue-50 transition-colors shadow-lg text-lg">
                申请免费试用
              </button>
              <button @click="goManual" class="w-full sm:w-auto px-8 py-3 bg-white/10 text-white font-semibold rounded-lg hover:bg-white/20 transition-colors border border-white/30">
                查看完整说明书
              </button>
            </div>
          </div>
...
const advantages = [
  ...
  {
    icon: 'M3 4h2l3 10h10l3-10h1M10 21a2 2 0 100-4 2 2 0 000 4zm8 0a2 2 0 100-4 2 2 0 000 4z',
    title: '最小分辨率1μm',
    desc: '满足高精度MEMS结构与微流控器件的制造需求'
  },
  {
  ...
  },
  {
    ...
    title: '基底附着力优异',
    desc: '通过多轮配方改进，增强附着力，适用于多层套刻工艺'
  },
  {
    icon: 'M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z',
    title: '高光学透过率',
    desc: '360nm以上具有非常高的光学透射率，适合较厚薄膜光刻'
  }
]
...
const priceComparison = [
  { spec: '500 mL', beizhou: '0.3 — 0.4万元', importPrice: '1.2 — 1.4万元' },
  { spec: '100 mL', beizhou: '0.7 — 0.9千元', importPrice: '3 — 4千元' },
